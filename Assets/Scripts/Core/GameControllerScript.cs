...
public class GameControllerScript : MonoBehaviour
{
	...
	{
		// [MOD] TEMPLATE: Both the "/7" part and the label are editable from the Inspector.
		int max = notebookCountMax > 0 ? notebookCountMax : 7;
		notebookCount.text = notebooks.ToString() + "/" + max + " " + notebookCountLabel;

		if (notebooks >= max & mode == "story")
		{
			ActivateFinaleMode();
		}
	}
	...
	// [MOD] TEMPLATE: When a notebook is grabbed, show a hit text instead of the math minigame.
	// The text is editable from the Inspector (hitTextMessage), stays fully visible for hitTextHoldTime,
	// then fades out over hitTextFadeTime. While open it plays hitSound and shakes.
	public void ShowHitText()
	{
		if (hitText == null)
		{
			hitText = FindSceneHitText();
		}
		if (hitText == null)
		{
			Debug.Log("[MOD] Hit-Text TMP object not found; hit text not shown.");
			return;
		}
		Debug.Log("[MOD] Showing hit text: '" + hitTextMessage + "' (hold=" + hitTextHoldTime + "s fade=" + hitTextFadeTime + "s)");
		hitText.text = hitTextMessage;
		hitText.color = Color.white;
		hitText.gameObject.SetActive(true);
		// [MOD] PNG support: if hitImageSprite is assigned, show it above the Hit-Text.
		if (hitImageSprite != null)
		{
			Image img = GetOrCreateHitImage();
			if (img != null)
			{
				img.sprite = hitImageSprite;
		...
		// [MOD] Shake the hit text (no need to permanently attach ShakingScript to the scene object).
		if (hitText.GetComponent<ShakingScript>() == null)
		{
			hitText.gameObject.AddComponent<ShakingScript>();
		}
		// [MOD] If no hit sound is assigned, auto-find the "BAL_Hi" clip in the scene.
		if (hitSound == null)
		{
			AudioClip[] clips = Resources.FindObjectsOfTypeAll<AudioClip>();
			foreach (AudioClip clip in clips)
			{
				if (clip != null && clip.name == "BAL_Hi")
				{
					hitSound = clip;
					break;
				}
		...
		if (hitSound != null)
		{
			audioDevice.PlayOneShot(hitSound);
		}
		if (hitCoroutine != null)
		{
			StopCoroutine(hitCoroutine);
		}
		hitCoroutine = StartCoroutine(HitTextFadeOut());
		// [MOD] TEMPLATE: The math game is bypassed, so DeactivateLearningGame is never called;
		// the quarter and all-notebooks rewards were moved here.
		if (notebooks == 1 & !spoopMode) // Give a quarter as a reward for the first notebook
		{
			quarter.SetActive(true);
			tutorBaldi.PlayOneShot(aud_Prize);
		}
		else if (notebooks >= notebookCountMax) // Play the "All Notebooks" sound on the final notebook
		{
			audioDevice.PlayOneShot(aud_AllNotebooks, 0.8f);
		}
	}
	// [MOD] Finds the "Hit-Text" TMP object in the scene even if it is inactive.
	private TMP_Text FindSceneHitText()
	{
		Canvas[] canvases = FindObjectsOfType<Canvas>();
		foreach (Canvas c in canvases)
		{
			Transform t = c.transform.Find("Hit-Text");
			if (t == null) t = FindHitTextInChildren(c.transform);
			if (t != null)
			{
	...
	private Transform FindHitTextInChildren(Transform parent)
	{
		for (int i = 0; i < parent.childCount; i++)
		{
			Transform child = parent.GetChild(i);
			if (child.name == "Hit-Text") return child;
			Transform result = FindHitTextInChildren(child);
			if (result != null) return result;
		}
		return null;
	}
	// [MOD] Creates/finds the Image used to show a sprite (PNG) above the Hit-Text.
	private Image GetOrCreateHitImage()
	{
		if (hitImage != null)
		{
			return hitImage;
		}
		Transform existing = hitText.transform.Find("Hit-Image");
		if (existing != null)
		{
			hitImage = existing.GetComponent<Image>();
			return hitImage;
		}
		GameObject go = new GameObject("Hit-Image");
		go.transform.SetParent(hitText.transform, false);
	...
	// [MOD] Shows the hit text, then fades it out.
	private IEnumerator HitTextFadeOut()
	{
		float holdTime = Mathf.Max(0f, hitTextHoldTime);
		float fadeTime = Mathf.Max(0f, hitTextFadeTime);
		float elapsed = 0f;
		...
		while (elapsed < fadeTime)
		{
			...
			if (hitText != null)
			{
				hitText.color = new Color(1f, 1f, 1f, 1f - t);
			}
			if (hitImage != null)
			{
				hitImage.color = new Color(1f, 1f, 1f, 1f - t);
			}
			elapsed += Time.unscaledDeltaTime;
			yield return null;
		}
	...
	// [MOD] Chase music: starts when spoopMode begins. Uses `chaseMusic` if assigned in the Inspector, otherwise loads Resources/Music/chase1.
	public AudioClip chaseMusic; // [MOD] Optional chase music clip, assignable from the Inspector.
	private void StartChaseMusic()
	{
		if (chaseAudioSource == null)
		{
			GameObject chaseAudioObject = new GameObject("ChaseMusic");
		...
		AudioClip chaseClip = chaseMusic != null ? chaseMusic : Resources.Load<AudioClip>("Music/chase1");
		if (chaseClip != null)
		{
	...
	public void DeactivateLearningGame(GameObject subject)
	{
		...
		else if (notebooks >= notebookCountMax & mode == "story") // Plays the all notebooks sound
		{
			audioDevice.PlayOneShot(aud_AllNotebooks, 0.8f);
		}
	}
	...
	// [MOD] TEMPLATE: Notebook counter settings, editable from the Inspector. notebookCountMax = the "/7" part, notebookCountLabel = the word after it.
	public int notebookCountMax = 7;
	public string notebookCountLabel = "Notebooks";
	public GameObject pauseMenu;
	// [MOD] TEMPLATE: Hit text settings shown when a notebook is grabbed.
	public TMP_Text hitText; // [MOD] If not assigned, the "Hit-Text" object in the scene HUD is auto-found.
	public string hitTextMessage = "SEN CİDDİ MİSİN?"; // [MOD] Text shown on screen (edit from the Inspector).
	public AudioClip hitSound; // [MOD] Sound played when the hit text opens (assign from the Inspector).
	public Sprite hitImageSprite; // [MOD] PNG: if assigned, shown above the hit text (assign from the Inspector).
	public float hitTextHoldTime = 2f; // [MOD] How long the hit text stays fully visible.
	public float hitTextFadeTime = 0f; // [MOD] Zero keeps the total visible duration at exactly two seconds.
	...
	private AudioSource chaseAudioSource; // [MOD] Chase music audio source.
}
